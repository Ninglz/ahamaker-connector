...
#
# 脚本会自动完成：
#   1. 从远程镜像下载 wechat_connector.py（纯标准库，无需 pip）
#   2. 生成配置文件 /etc/ahamaker-connector.env（权限 600；未提供令牌时自动生成）
#   3. 注册并启动 systemd 服务（开机自启）
#   4. 健康检查 + 自检
#   5. 打印「连接器地址 / 访问令牌 / 服务器公网 IP」——填回啊哈创作 App 即可
#
...
if [ "${1:-}" = "--uninstall" ]; then
  ...
  echo "已停止并移除 ahamaker-connector 服务；目录 $INSTALL_DIR 保留（可手动删除）。"
  exit 0
fi
